    ...
    with col1:
        ...
        if mobile_app_url:
            ...
            if st.button(
                ...
                st.query_params["view"] = "instrukcja"
                st.rerun()
        else:
            st.info(
...
def render_installation_view(training_code):
    mobile_app_url = get_mobile_app_url()
...
7. Wpisz swój pseudonim oraz kod śledzenia: **{training_code}**.
...
if st.query_params.get("view") == "instrukcja":
    render_installation_view(training_code)
    st.stop()
